...
from application_core.users.constants import (
    ...
    USER_PATRONYMIC_MAX_LENGTH, USER_PHONE_NUMBER_MAX_LENGTH,
)
...
class Profiles(Base):
    ...
    phone_number: Mapped[str] = mapped_column("phone_number", String(USER_PHONE_NUMBER_MAX_LENGTH), nullable=True)
    verified_email: Mapped[str] = mapped_column("verified_email", String(USER_EMAIL_MAX_LENGTH), nullable=True)
    verified_phone_number: Mapped[str] = mapped_column(
        "verified_phone_number", String(USER_PHONE_NUMBER_MAX_LENGTH),
        nullable=True
    )
